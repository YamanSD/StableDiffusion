...
def generate(
        ...
        do_cfg: bool = True,
        cfg_scale: float = 7.5,
        sampler_name: Literal['ddpm'] = "ddpm",
    ...
    with torch.no_grad():
        ...
        if idle_device:
            to_idle: Callable[[torch.Tensor], torch.Tensor] = lambda x: x.to(idle_device)
        else:
            to_idle: Callable[[torch.Tensor], torch.Tensor] = lambda x: x
